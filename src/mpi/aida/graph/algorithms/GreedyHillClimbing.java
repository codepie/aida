...
public class GreedyHillClimbing {
	private static final Logger logger = LoggerFactory
			.getLogger(GreedyHillClimbing.class);
	...
	public GreedyHillClimbing(Graph graph, Set<Integer> mnodes,
			Set<Integer> enodes, boolean[] rem, int maxCombinations) {
	...
	public double computeWeight(Map<Integer, Integer> currentSolution) {
		...
		for (int mentionNodeId : currentSolution.keySet()) {
			...
			if (inputGraph.isRemoved(mentionNodeId))
				continue;
			...
			for (int i = successorsMap.size(); i-- > 0;) {
				...
				if (inputGraph.isRemoved(successorId))
					continue;
		...
		for (int entityId : entities) {
			...
			if (inputGraph.isRemoved(entityId))
				continue;
	...
	public Map<Integer, Integer> runExhaustive(String graphName)
		...
		for (int mentionNodeId : mentionNodes) {
			...
			if (actualOutdegree > 0) {
				...
				actualMentionSuccessors.put(mentionNodeId,
						actualSuccessors.toArray(new Integer[actualOutdegree]));
		...
		if (possibleCombinations > this.maxCombinationsExhaustive) {
			...
			logger.debug("The combinations to check are "
					+ possibleCombinations);
			logger.debug("Applying local search");

			GraphTracer.gTracer.addStat(graphName,
					"Exhaustive search not applied, too many combinations",
					Long.toString(possibleCombinations));

			return null;
		}

		logger.debug("Number of possible combinations that need to be checked: "
						+ possibleCombinations);

		GraphTracer.gTracer.addStat(graphName,
				"Exhaustively searching number of combinations",
				Long.toString(possibleCombinations));

		Integer[] mentionIds = actualMentionDegrees.keySet().toArray(
				new Integer[actualMentionDegrees.keySet().size()]);
		if (possibleCombinations > 0) {
			Map<Integer, Integer> currentConfiguration = new HashMap<Integer, Integer>();
			GraphConfiguration gc = permuteRecursive(mentionIds, 0,
		...
		GraphTracer.gTracer.addStat(graphName,
				...
				Double.toString(bestTotalWeight));

		return bestChoice;
	}

	private GraphConfiguration permuteRecursive(Integer[] mentionIds,
			int currentMentionIndex, Map<Integer, Integer> currentConfiguration) {
		if (currentMentionIndex < mentionIds.length) {
			int currentMention = mentionIds[currentMentionIndex];
			GraphConfiguration gc = new GraphConfiguration(null,
					Double.MIN_VALUE);
			currentMentionIndex++;

			for (int candidate : actualMentionSuccessors.get(currentMention)) {
				currentConfiguration.put(currentMention, candidate);
				GraphConfiguration newGc = permuteRecursive(mentionIds,
						currentMentionIndex, currentConfiguration);
			...
			GraphConfiguration gc = new GraphConfiguration(
					new HashMap<Integer, Integer>(currentConfiguration), weight);
			return gc;
		}
	}

	public HashMap<Integer, Integer> localSearch(int numberOfMoves)
			throws Exception {
	 ...
	 * @param graphName
	 * */
	public HashMap<Integer, Integer> localSearch(String graphName,
			int numberOfMoves) throws Exception {

		logger.debug("The starting graph has "
				+ inputGraph.getNodesCount() + " nodes and "
				+ inputGraph.getEdgesCount() + " arcs");
		...
		for (int mentionNodeId : mentionNodes) {
			...
			for (int i = successorsMap.size(); i-- > 0;) {
				...
				if (inputGraph.isRemoved(successor))
					continue;
		...
		GraphTracer.gTracer.addStat(graphName,
				"HillClimbing for total number of moves",
				Integer.toString(numberOfMoves));

		if (possibleCombinations > 0) {

			while (checkedCombinations.keySet().size() < numberOfMoves
					&& checkedCombinations.keySet().size() < possibleCombinations) {
				...
				for (int chosenMention : mentionList) {
					...
					int chosenEntityIndex = generator
							.nextInt(actualMentionDegrees.get(chosenMention));
					int chosenEntity = actualMentionSuccessors
							.get(chosenMention)[chosenEntityIndex];
		...
		GraphTracer.gTracer.addStat(graphName,
				...
				Integer.toString(checkedCombinations.size()));

		GraphTracer.gTracer.addStat(graphName,
				"HillClimbing got final objective score",
				Double.toString(currentSolution));

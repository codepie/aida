...
public class WeightComputation {
  ...
  public static double computeNPMI(int a, int b, int ab, int total) {
    ...
    double npmi = (Math.log(jointProb / (aProb * bProb)))
        / -Math.log(jointProb);
   ...
   */
  public static double computeMI(int a, int b, int ab, int total, 
      boolean normalize) {
    assert a < total;
    assert b < total;
    assert ab < total;
  ...
  private static double getPNdk(double pc, double koc, double ic) {
    ...
    if (numerator <= 0.0) { // FIXME this skips keywords where the frequencies are borked
      numerator = 1.0; // otherwise undefined
    }
